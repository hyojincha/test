...
#define TEST_DEVICE_NAME     "ESP_BT05"
...
static uint8_t char1_str[] = "Tx&Rx";
...
static uint8_t adv_service_data[6] = {
    /* LSB <--------------------------------------------------------------------------------> MSB */

    0x00, 0x00, 0x00, 0x00, 0xB0, 0x00,
};
static uint8_t manufacturer_data[9] = {
    0xff,0x48,0x4d,0x88,0x25,0x83,0xf0,0xd6,0x96
};
...
static esp_ble_adv_data_t adv_data = {
    ...
    .p_manufacturer_data = manufacturer_data, //&test_manufacturer[0],
    .service_data_len = 0,
    .p_service_data = NULL,
...
static esp_ble_adv_data_t scan_rsp_data = {
    ...
    .service_data_len = sizeof(adv_service_data),
    .p_service_data = adv_service_data,
    .service_uuid_len = 0,
    .p_service_uuid = NULL,
    .flag = (ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT),
};
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t* param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        if (ret) {
            ESP_LOGE(GATTS_TAG, "config raw scan rsp data failed, error code = %x", ret);
        }
        adv_config_done |= scan_rsp_config_flag;
        ...
        if (!param->write.is_prep) {
            ...
            if (gl_profile_tab.descr_handle == param->write.handle && param->write.len == 2) {
                ...
                if (descr_value == 0x0001) {
                    ...
                    esp_log_buffer_hex(GATTS_TAG, param->write.value, param->write.len);
                    char str[100];
                    snprintf(str, param->write.len, "%s", param->write.value);
                    ESP_LOGI(GATTS_TAG, "%s", str);
                }
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        conn_params.max_int = 10;    // max_int = 0x20*1.25ms = 40ms
        conn_params.min_int = 8;    // min_int = 0x10*1.25ms = 20ms
...
void ble_send_str(char* str) {

    char* ptr = str;
    while (strlen(ptr) >= 20) {
        //ESP_LOGI(GATTS_TAG,"strlen=%d",(uint16_t)strlen(ptr));
        esp_ble_gatts_send_indicate(gl_profile_tab.gatts_if,
